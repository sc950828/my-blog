...
[TypeScript 文档 1](https://www.tslang.cn/docs/home.html)

[TypeScript 文档 2](https://ts.xcatliu.com/introduction/what-is-typescript.html)
...
元组 元组类型允许表示一个已知元素数量和类型的数组，各元素的类型不必相同 但是顺序和个数不能变
  let x: [string, number];
  x = ['hello', 10];
...
枚举 enum类型是对JavaScript标准数据类型的一个补充
  ...
  let c: Color = Color.Green; //返回的是下标 类似数组下标从0开始 也可以赋初始值Red=10等这样返回的就是11
  我们还可以设置枚举的值是字符串enum Direction {LEFT = "left",RIGHT = "right",}
  我们还可以通过下标访问Color[0] === Red
...
### 接口 interface 使用 implements 实现接口。 使用 extends 继承接口，可以多继承。
...
### 类 class 使用 extends 实现继承，使用 implements 实现接口。可以实现多个接口
...
抽象类使用abstract关键词定义
抽象类做为其它派生类的基类使用,它们不能直接被实例化,不同于接口,抽象类可以包含成员的实现细节。
...
- public 默认 成员是可以被内部访问和被外部访问。
- protected 只可以被类的内部以及类的子类访问。
- private 只可以被类的内部访问。私有 private 的变量在外面如果需要访问 就需要提供 get set 方法。
