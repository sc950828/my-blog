...
class ArticleCategoryOperate extends PureComponent {
  ...
  onFinish = (formData) => {
    ...
    formData.goArticleCategory = () => {
      ...
      const pageParams = { pathname: "/articleCategory" };
      if (params) {
        pageParams.state = params;
      }
      history.push(pageParams);
    };
    if (id) {
      formData.id = id;
          ...
          <Form.Item
            ...
            rules={[
              ...
              { whitespace: true, message: "请输入正确的分类标题" },
              {
                type: "string",
                max: 20,
                message: "请输入正确的分类标题(不超过20个字符)",
              },
            ]}
          >
            <Input
              disabled={isLook}
              placeholder="请输入文章分类标题，不超过20个字符"
            ></Input>
          </Form.Item>
          <Form.Item
            label="封面图链接"
            name="banner"
            rules={[{ required: true, message: "请输入封面图链接" }]}
          >
